...
- **Self-service flows**, API-token and browser-cookie (CSRF-protected) alike: registration with verify-before-first-session policy, login, recovery (one-time codes, anti-enumeration, revokes other sessions, never bypasses MFA), verification, logout. Browser flows are content-negotiated: a client that prefers HTML — or posts a plain form — is driven by redirects to the tenant's own screens, carrying the flow to render and the messages to show, while JSON clients get the flow as JSON.
...
- **Passwordless first factor** (opt-in per tenant, `first_factor: ["code"]`): sign in by proving a phone or email address with a one-time code, no password credential anywhere — for phone-first products where a password is the one credential nobody wants. The send step answers identically for identifiers that do not exist, so it is no enumeration oracle.
